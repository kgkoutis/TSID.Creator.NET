...
public class TsidFactory00256Test : TsidFactory00000Test {
	...
	private static readonly int NODE_BITS = 8;
	private static readonly int COUNTER_BITS = 14;
	...
	public void TestGetTsid256Parallel() {
		...
		for (int i = 0; i < MultiplePasses; i++) {
			TsidFactory factory = TsidFactory.GetBuilder().WithNode(i).WithNodeBits(NODE_BITS).WithRandom(random).Build();
